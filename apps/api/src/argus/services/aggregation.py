...
from argus.domain.models import Decision, DecisionEvidence, Evidence, Company
...
OPEN_DECISION_KEY = "decision:open:{company_id}:{camera_id}:{region_key}"
...
class AggregationService:
    async def aggregate(self, evidence_id: UUID) -> Decision:
        async with company_session(None, UserRole.ROOT.value) as session:
            ...
            await session.execute(
                text("SELECT set_config('app.current_company_id', :tid, true)"),
                {"tid": str(evidence.company_id)},
            )
            await session.execute(
            ...
            company = await session.get(Company, evidence.company_id)
            if company is None:
                raise ValueError(f"Company not found: {evidence.company_id}")

            decision = await self._merge_or_create_decision(session, evidence, company)
            old_state = decision.state
            await self._apply_state_transitions(session, decision, company, old_state)
    ...
    async def _merge_or_create_decision(
        ...
        if decision and evidence.captured_at <= decision.window_end:
            ...
            decision = await self._create_decision(session, evidence, company)
            await set_key(
                open_key,
                str(decision.id),
                ex=company.aggregation_window_secs + 300,
            )
            await self._link_evidence(session, decision, evidence)
    ...
    async def _create_decision(
        ...
        window_end = window_start + timedelta(seconds=company.aggregation_window_secs)
        decision = Decision(
            company_id=evidence.company_id,
            camera_id=evidence.camera_id,
    ...
    async def _apply_state_transitions(
        ...
        new_state = self._compute_state(decision, company)
    ...
    def _compute_state(self, decision: Decision, company: Company) -> DecisionState:
        if decision.cumulative_severity >= company.warning_threshold:
            return DecisionState.WARNING
        if decision.evidence_count >= company.weird_threshold:
