...
from argus.integrations.auth0 import create_mock_m2m_token  # noqa: E402
from tests.helpers import bearer, session_token  # noqa: E402
...
async def test_sso_token_rejected(
    ...
    # A user session token must never satisfy the edge M2M gate.
    sso_token = await session_token(UserRole.OPERATOR, SEED_COMPANY_ID)
    response = await client.post(
        "/v1/ingest/sequences",
        json=sample_payload,
...
@pytest.mark.asyncio
async def test_failed_ingestion_can_be_retried_after_mode_becomes_active(
    client: AsyncClient,
    sample_payload: dict,
    m2m_token: str,
    active_camera_mode,
    ...
    ingestion_id = str(uuid.uuid4())
    payload = {**sample_payload, "ingestion_id": ingestion_id}
    key = f"ingest:idem:{SEED_COMPANY_ID}:{ingestion_id}"
    await delete_key(key)
    await delete_key(f"camera:active_mode:{SEED_CAMERA_ID}")

    first = await client.post(
        "/v1/ingest/sequences", json=payload, headers=_auth_headers(m2m_token)
    )
    assert first.status_code == 400
    assert not await get_redis().exists(key)

    await get_redis().set(f"camera:active_mode:{SEED_CAMERA_ID}", SEED_MODE_ID)
