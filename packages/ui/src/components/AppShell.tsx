...
export function AppShell({ brand, meta, actions, children, wide }: AppShellProps) {
  return (
    <div className="argus-shell">
      ...
      <main className={['argus-shell__body', wide ? 'argus-shell__body--wide' : ''].filter(Boolean).join(' ')}>
        {children}
      </main>
    </div>
  );
